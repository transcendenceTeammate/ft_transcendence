...
export default class OnlineGame extends Game {
    ...
    processPlayerInput(deltaTime) {
        // Only process if we're a player (not spectator)
        if (!this.playerNumber) return;
        
        // Process movement inputs with priority
        // This ensures paddle movement remains responsive
        if (this.playerNumber === 1) {
            // Player 1 controls left paddle
            if (this.upPressed) {
                this.player1Y = Math.max(0, this.player1Y - this.paddleSpeed);
            }
            if (this.downPressed) {
                this.player1Y = Math.min(this.canvas.height - this.paddleHeight, this.player1Y + this.paddleSpeed);
            }
        } else if (this.playerNumber === 2) {
            // Player 2 controls right paddle
            if (this.upPressed) {
                this.player2Y = Math.max(0, this.player2Y - this.paddleSpeed);
            }
            if (this.downPressed) {
                this.player2Y = Math.min(this.canvas.height - this.paddleHeight, this.player2Y + this.paddleSpeed);
            }
        }
    }
    ...
    interpolateOpponentPaddle(deltaTime) {
        // Use a fixed interpolation factor regardless of latency
        // This ensures consistent movement speed
        const factor = 0.2; // Fixed factor for more predictable movement
        
        if (this.playerNumber === 1) {
            // We control player 1, interpolate player 2
            const diff = this.opponentPaddleTarget - this.player2Y;
            if (Math.abs(diff) > 0.1) {
                // Apply interpolation with deltaTime to keep consistent speed
    ...
    reconcilePlayerPaddle(serverPaddleY, playerNum) {
        // Get locally predicted paddle position
        const clientPaddleY = playerNum === 1 ? this.player1Y : this.player2Y;
        
        // Check if there's a significant discrepancy (more than threshold pixels)
        if (Math.abs(serverPaddleY - clientPaddleY) > GameConstants.SERVER_RECONCILIATION_THRESHOLD) {
            // Reset to server state with a slight blend for smoothness
            if (playerNum === 1) {
                this.player1Y = serverPaddleY * 0.7 + clientPaddleY * 0.3;
            } else {
                this.player2Y = serverPaddleY * 0.7 + clientPaddleY * 0.3;
            }
        }
    }
